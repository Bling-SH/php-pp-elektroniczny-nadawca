...
class UpdateAccountResponse extends AbstractStructBase
{
    ...
    public static function validateErrorForArrayConstraintsFromSetError(array $values = [])
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $updateAccountResponseErrorItem) {
            // validation for constraint: itemType
            if (!$updateAccountResponseErrorItem instanceof \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType) {
                $invalidValues[] = is_object($updateAccountResponseErrorItem) ? get_class($updateAccountResponseErrorItem) : sprintf('%s(%s)', gettype($updateAccountResponseErrorItem), var_export($updateAccountResponseErrorItem, true));
            }
        }
     ...
     * @param \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType[] $error
     *
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\UpdateAccountResponse
     */
