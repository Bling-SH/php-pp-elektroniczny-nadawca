...
class UpdateEnvelopeBuforResponse extends AbstractStructBase
{
    ...
    public static function validateErrorForArrayConstraintsFromSetError(array $values = [])
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $updateEnvelopeBuforResponseErrorItem) {
            // validation for constraint: itemType
            if (!$updateEnvelopeBuforResponseErrorItem instanceof \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType) {
                $invalidValues[] = is_object($updateEnvelopeBuforResponseErrorItem) ? get_class($updateEnvelopeBuforResponseErrorItem) : sprintf('%s(%s)', gettype($updateEnvelopeBuforResponseErrorItem), var_export($updateEnvelopeBuforResponseErrorItem, true));
            }
        }
     ...
     * @param \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType[] $error
     *
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\UpdateEnvelopeBuforResponse
     */
