...
class ZamowKurieraResponse extends AbstractStructBase
{
    ...
    public static function validateErrorForArrayConstraintsFromSetError(array $values = [])
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $zamowKurieraResponseErrorItem) {
            // validation for constraint: itemType
            if (!$zamowKurieraResponseErrorItem instanceof \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType) {
                $invalidValues[] = is_object($zamowKurieraResponseErrorItem) ? get_class($zamowKurieraResponseErrorItem) : sprintf('%s(%s)', gettype($zamowKurieraResponseErrorItem), var_export($zamowKurieraResponseErrorItem, true));
            }
        }
