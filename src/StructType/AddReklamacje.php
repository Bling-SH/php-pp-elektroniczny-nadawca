...
class AddReklamacje extends AbstractStructBase
{
     ...
     *
     * @param \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ReklamowanaPrzesylkaType[] $reklamowanaPrzesylka
     */
     ...
     */
    public static function validateReklamowanaPrzesylkaForArrayConstraintsFromSetReklamowanaPrzesylka(array $values = [])
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $addReklamacjeReklamowanaPrzesylkaItem) {
            // validation for constraint: itemType
            if (!$addReklamacjeReklamowanaPrzesylkaItem instanceof \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ReklamowanaPrzesylkaType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The reklamowanaPrzesylka property can only contain items of type \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ReklamowanaPrzesylkaType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ReklamowanaPrzesylkaType[] $reklamowanaPrzesylka
     *
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\AddReklamacje
     */
     ...
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\AddReklamacje
     */
    public function addToReklamowanaPrzesylka(\Bling\PocztaPolska\ElektronicznyNadawca\StructType\ReklamowanaPrzesylkaType $item)
    {
        // validation for constraint: itemType
        if (!$item instanceof \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ReklamowanaPrzesylkaType) {
            throw new \InvalidArgumentException(sprintf('The reklamowanaPrzesylka property can only contain items of type \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ReklamowanaPrzesylkaType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(500)
