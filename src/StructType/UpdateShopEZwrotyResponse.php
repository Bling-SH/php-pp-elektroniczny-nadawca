...
class UpdateShopEZwrotyResponse extends AbstractStructBase
{
    ...
    public static function validateErrorForArrayConstraintsFromSetError(array $values = [])
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $updateShopEZwrotyResponseErrorItem) {
            // validation for constraint: itemType
            if (!$updateShopEZwrotyResponseErrorItem instanceof \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType) {
                $invalidValues[] = is_object($updateShopEZwrotyResponseErrorItem) ? get_class($updateShopEZwrotyResponseErrorItem) : sprintf('%s(%s)', gettype($updateShopEZwrotyResponseErrorItem), var_export($updateShopEZwrotyResponseErrorItem, true));
            }
        }
     ...
     * @param \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType[] $error
     *
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\UpdateShopEZwrotyResponse
     */
