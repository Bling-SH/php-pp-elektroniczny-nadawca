...
class ChangePasswordResponse extends AbstractStructBase
{
    ...
    public static function validateErrorForArrayConstraintsFromSetError(array $values = [])
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $changePasswordResponseErrorItem) {
            // validation for constraint: itemType
            if (!$changePasswordResponseErrorItem instanceof \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType) {
                $invalidValues[] = is_object($changePasswordResponseErrorItem) ? get_class($changePasswordResponseErrorItem) : sprintf('%s(%s)', gettype($changePasswordResponseErrorItem), var_export($changePasswordResponseErrorItem, true));
            }
        }
