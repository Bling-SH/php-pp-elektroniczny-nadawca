...
class ZwrotDokumentowKurierskaType extends AbstractStructBase
{
     ...
     */
    public function __construct($rodzajPocztex = null, $rodzajPaczka = null, \Bling\PocztaPolska\ElektronicznyNadawca\StructType\RodzajListType $rodzajList = null)
    {
        $this
     ...
     *
     * @uses \Bling\PocztaPolska\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowKurierskaType::valueIsValid()
     * @uses \Bling\PocztaPolska\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowKurierskaType::getValidValues()
     *
     * @throws \InvalidArgumentException
     *
     * @param string $rodzajPocztex
     *
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ZwrotDokumentowKurierskaType
     */
    public function setRodzajPocztex($rodzajPocztex = null)
    {
        // validation for constraint: enumeration
        if (!\Bling\PocztaPolska\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowKurierskaType::valueIsValid($rodzajPocztex)) {
            throw new \InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Bling\PocztaPolska\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowKurierskaType', is_array($rodzajPocztex) ? implode(', ', $rodzajPocztex) : var_export($rodzajPocztex, true), implode(', ', \Bling\PocztaPolska\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowKurierskaType::getValidValues())), __LINE__);
        }
     ...
     * @return string A non-empty message if the values does not match the validation rules
     */
    public function validateRodzajPaczkaForChoiceConstraintsFromSetRodzajPaczka($value)
    {
        $message = '';
        if (is_null($value)) {
            return $message;
     ...
     * @throws \InvalidArgumentException
     *
     * @uses \Bling\PocztaPolska\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowPaczkowaType::valueIsValid()
     * @uses \Bling\PocztaPolska\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowPaczkowaType::getValidValues()
     *
     * @throws \InvalidArgumentException
     *
     * @param string $rodzajPaczka
     ...
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ZwrotDokumentowKurierskaType
     */
    public function setRodzajPaczka($rodzajPaczka = null)
    {
        // validation for constraint: enumeration
        if (!\Bling\PocztaPolska\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowPaczkowaType::valueIsValid($rodzajPaczka)) {
            throw new \InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Bling\PocztaPolska\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowPaczkowaType', is_array($rodzajPaczka) ? implode(', ', $rodzajPaczka) : var_export($rodzajPaczka, true), implode(', ', \Bling\PocztaPolska\ElektronicznyNadawca\EnumType\TerminZwrotDokumentowPaczkowaType::getValidValues())), __LINE__);
        }
        // validation for constraint: choice(rodzajPocztex, rodzajPaczka, rodzajList)
