...
class AddZalacznikDoReklamacjiResponse extends AbstractStructBase
{
    ...
    public static function validateErrorForArrayConstraintsFromSetError(array $values = [])
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $addZalacznikDoReklamacjiResponseErrorItem) {
            // validation for constraint: itemType
            if (!$addZalacznikDoReklamacjiResponseErrorItem instanceof \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType) {
                $invalidValues[] = is_object($addZalacznikDoReklamacjiResponseErrorItem) ? get_class($addZalacznikDoReklamacjiResponseErrorItem) : sprintf('%s(%s)', gettype($addZalacznikDoReklamacjiResponseErrorItem), var_export($addZalacznikDoReklamacjiResponseErrorItem, true));
            }
        }
     ...
     * @param \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType[] $error
     *
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\AddZalacznikDoReklamacjiResponse
     */
