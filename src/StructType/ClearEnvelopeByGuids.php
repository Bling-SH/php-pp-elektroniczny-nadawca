...
class ClearEnvelopeByGuids extends AbstractStructBase
{
    ...
    public static function validateGuidForArrayConstraintsFromSetGuid(array $values = [])
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $clearEnvelopeByGuidsGuidItem) {
            // validation for constraint: itemType
            if (!is_string($clearEnvelopeByGuidsGuidItem)) {
                $invalidValues[] = is_object($clearEnvelopeByGuidsGuidItem) ? get_class($clearEnvelopeByGuidsGuidItem) : sprintf('%s(%s)', gettype($clearEnvelopeByGuidsGuidItem), var_export($clearEnvelopeByGuidsGuidItem, true));
            }
        }
    ...
    public static function validateGuidForLengthConstraintFromSetGuid($values)
    {
        ...
        foreach ($values as $clearEnvelopeByGuidsGuidItem) {
            ...
            if (32 !== mb_strlen($clearEnvelopeByGuidsGuidItem)) {
                $invalidValues[] = var_export($clearEnvelopeByGuidsGuidItem, true);
            }
        }
        if (!empty($invalidValues)) {
     ...
     * @param string[] $guid
     *
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ClearEnvelopeByGuids
     */
