...
class UpdateProfilResponse extends AbstractStructBase
{
    ...
    public static function validateErrorForArrayConstraintsFromSetError(array $values = [])
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $updateProfilResponseErrorItem) {
            // validation for constraint: itemType
            if (!$updateProfilResponseErrorItem instanceof \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType) {
                $invalidValues[] = is_object($updateProfilResponseErrorItem) ? get_class($updateProfilResponseErrorItem) : sprintf('%s(%s)', gettype($updateProfilResponseErrorItem), var_export($updateProfilResponseErrorItem, true));
            }
        }
     ...
     * @param \Bling\PocztaPolska\ElektronicznyNadawca\StructType\ErrorType[] $error
     *
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\UpdateProfilResponse
     */
