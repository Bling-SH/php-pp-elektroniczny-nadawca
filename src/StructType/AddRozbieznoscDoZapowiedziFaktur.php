...
class AddRozbieznoscDoZapowiedziFaktur extends AbstractStructBase
{
    ...
    /**
     * This method is responsible for validating the values passed to the setRozbieznosciZipFile method
     ...
     */
    public static function validateRozbieznosciZipFileForArrayConstraintsFromSetRozbieznosciZipFile(array $values = [])
    {
        $message = '';
        $invalidValues = [];
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The rozbieznosciZipFile property can only contain items of type base64Binary, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param string[] $rozbieznosciZipFile
     *
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\AddRozbieznoscDoZapowiedziFaktur
     */
    public function setRozbieznosciZipFile(array $rozbieznosciZipFile = [])
    {
        // validation for constraint: array
        if ('' !== ($rozbieznosciZipFileArrayErrorMessage = self::validateRozbieznosciZipFileForArrayConstraintsFromSetRozbieznosciZipFile($rozbieznosciZipFile))) {
     ...
     * @return \Bling\PocztaPolska\ElektronicznyNadawca\StructType\AddRozbieznoscDoZapowiedziFaktur
     */
    public function addToRozbieznosciZipFile($item)
    {
        // validation for constraint: itemType
        if (!is_string($item)) {
            throw new \InvalidArgumentException(sprintf('The rozbieznosciZipFile property can only contain items of type base64Binary, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(10)
